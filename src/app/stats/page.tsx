...
import { Team, Match, Player, PlayerSeasonStats } from '@/types';
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <div className="text-xs text-muted-foreground">
                              #{player.playerNumber} · {player.position === 'goalkeeper' ? '门将' : player.position === 'defender' ? '后卫' : player.position === 'midfielder' ? '中场' : '前锋'}
                            </div>
                          </div>
                        </div>
...
function RankingCard({ title, players, statKey, statLabel, color }: any) {
  if (players.length === 0) {
    return (
      <Card>
        <CardContent className="flex flex-col items-center justify-center py-12">
          ...
          {players.map((player: PlayerSeasonStats, index: number) => (
            <div key={player.playerId} className="flex items-center justify-between p-4 rounded-lg border bg-background hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
              <div className="flex items-center gap-4">
                <div className="flex items-center justify-center w-10 h-10 rounded-full bg-gradient-to-br from-slate-100 to-slate-200 dark:from-slate-700 dark:to-slate-800 font-bold text-lg">
                  {index + 1}
                </div>
                <div>
                  <div className="font-semibold text-base">{player.playerName}</div>
                  <div className="text-xs text-muted-foreground">
                    #{player.playerNumber} · {player.matchesPlayed}场比赛
                  </div>
                </div>
              </div>
